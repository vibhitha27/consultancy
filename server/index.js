...
// Update the email sending route
app.post('/api/orders/send-confirmation', authenticateToken, async (req, res) => {
  try {
    const { customerEmail, orderDetails } = req.body;

    // Log email configuration status (without sensitive data)
    console.log('Email configuration status:', {
      hasEmailUser: !!process.env.EMAIL_USER_SEND,
      hasEmailPass: !!process.env.EMAIL_PASS_SEND,
      host: 'smtp.gmail.com',
      port: 465
    });

    if (!process.env.EMAIL_USER_SEND || !process.env.EMAIL_PASS_SEND) {
    ...
    const mailOptions = {
      from: {
        name: 'Sri Jeyam Tyres',
        address: process.env.EMAIL_USER_SEND
      },
      to: customerEmail,
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
            ...
            <tfoot>
              <tr>
                <td colspan="2" style="padding: 10px; text-align: right; border: 1px solid #ddd; font-weight: bold;">Total:</td>
                <td style="padding: 10px; border: 1px solid #ddd; font-weight: bold;">₹${orderDetails.total.toFixed(2)}</td>
              </tr>
            </tfoot>
          </table>
          
          <p><strong>Order ID:</strong> ${orderDetails.orderId}</p>
          <p><strong>Estimated Delivery Date:</strong> ${orderDetails.estimatedDelivery || '3-5 business days'}</p>
          
          <p>If you have any questions about your order, please contact our customer support team.</p>
          
          <p>Best regards,</p>
          <p>The Sri Jeyam Tyres Team</p>
